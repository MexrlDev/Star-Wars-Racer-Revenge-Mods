"""
EgyDevTeam NES Launcher - Androido Port
# Android port by MexrlDev

Place your PS4/PS5 IP between " " below and run the script.
ROMs (.nes) should be placed in a folder named 'roms' next to this script.
"""
...
# ========== CONFIGURE THESE ==========
DEFAULT_PS5_IP = ""     # Change to your PS4/PS5 IP address
DEFAULT_ROMS_DIR = "roms"             # Folder containing .nes ROM files
DEFAULT_LAUNCHER = "nes.lua"          # Lua payload filename
PAYLOAD_PORT = 9026
FTP_PORT = 1337
# =====================================

def find_file(name, subdirs=["payloads", "lua"]):
    """Locate a file in the current directory or common subdirectories."""
...
def load_binary_file(filepath):
    ...
    if not path:
        ...
        print(f"  Searched in: current folder, ./payloads/, ./lua/")
        return None
    try:
...
def send_payload(host, filepath):
    """Send the launcher payload to the PS5 via TCP."""
...
def wait_for_ftp(host, port, timeout=50):
    """Check if the FTP server is alive by looking for a '220' banner."""
...
def send_site_exit(host, port):
    """Send SITE EXIT command to cleanly shut down the FTP server."""
    try:
        ftp = FTP()
...
def upload_roms(host, roms, port=FTP_PORT):
    """Upload ROM files to the PS5's FTP server with progress feedback."""
    total = len(roms)
...
def scan_roms(roms_dir, extensions):
    """Return a sorted list of ROM files with matching extensions."""
...
def main():
    # Parse command-line arguments manually (no argparse needed)
    ...
    print("EgyDevTeam NES Launcher - Android Port")
    print(f"  PS4/PS5: {ps5_ip}  ROMs folder: {roms_dir}")
    print()
    ...
    # Retry once after a short delay if not ready immediately
