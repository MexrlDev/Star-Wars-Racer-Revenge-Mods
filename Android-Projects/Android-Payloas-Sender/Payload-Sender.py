# Star Wars Revenge Android Payload sender
# Please create file called payloads next to the python file and put payloads in it if wasnt already there..
...
DEFAULT_PORT = 9026
DEFAULT_PAYLOAD_NAME = "hello.lua"   # <<< Change to payload name!! Can be .lua, .bin, .elf, etc.
# =====================================
...
def load_payload_file(filename):
    ...
    if not path:
        print(f"Ã¢ÂÂ File not found: {filename}")
        print(f"  Looked in: current folder, ./payloads/, ./lua/")
        return None
    try:
        with open(path, 'rb') as f:   # binary mode
            content = f.read()
        print(f"Ã¢ÂÂ Loaded file: {path} ({len(content)} bytes)")
        return content
    except Exception as e:
        print(f"Ã¢ÂÂ Error reading {path}: {e}")
        return None
...
def send_payload(data, host, port):
    ...
    try:
        ...
        sock.sendall(data)
        print(f"Ã¢ÂÂ Sent {len(data)} bytes to {host}:{port}")
        return True
    except socket.error as e:
        print(f"Ã¢ÂÂ Socket error: {e}")
        if e.errno == 65:  # No route to host
            print("\n TIPS:")
            print("  - Is the PS4 on and connected to the same WiÃ¢ÂÂFi?")
            print("  - Is the PS4Ã¢ÂÂs IP address correct?")
...
def main():
    ...
    # Sends it
    print(f"\nÃ¢ÂÂ Target: {host}:{port}")
    print(f"Ã¢ÂÂ Payload: {payload_filename}")
